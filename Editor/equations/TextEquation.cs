...
namespace Editor
{
    public class TextEquation : EquationBase, ISupportsUndo
    {
        ...
        public override void ConsumeText(string text)
        {
            ...
            UndoManager.AddUndoAction(new TextAction(this, caretIndex, text, tempFormats, tempModes, new CharacterDecorationInfo[0])
            {
                UndoFlag = false
            });
        ...
        public override void ConsumeFormattedText(string text, int[] formats, EditorMode[] modes, CharacterDecorationInfo[] decorations, bool addUndo)
        {
            ...
            if (addUndo)
            {
                UndoManager.AddUndoAction(new TextAction(this, caretIndex, text, formats, modes, decorations ?? new CharacterDecorationInfo[0])
                {
                    UndoFlag = false
                });
            }
        ...
        class HeightMetrics
        {
            internal double TopExtra
            {
                get;
                set;
            }
            internal double UpperHalf
            {
                get;
                set;
            }
            internal double LowerHalf
            {
                get;
                set;
            }
        ...
        public void AddDecoration(CharacterDecorationType cdt, Position position, string sign)
        {
            if (cdt == CharacterDecorationType.None)
            {
                ...
                UndoManager.AddUndoAction(new DecorationAction(this, decoArray)
                {
                    UndoFlag = false
                });
